...
class TrackedResource:
    ...
    in_use: bool = False
    auto_cleanup_after: Optional[int] = None  # seconds
...
class FileContentResource(TrackedResource):
    ...
    content_ref: Optional[weakref.ref] = field(default=None, init=False, repr=False)
    
    def __post_init__(self):
        """Set up weak reference to content for memory tracking."""
        if self.content:
            self.content_ref = weakref.ref(self.content)
